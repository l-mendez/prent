...
// header) onto the session's auth attributes, where the dynamic persona resolver reads it
// (agent/instructions/persona.ts).
//
...
function prentModeAuth(): AuthFn<Request> {
  return (request) => {
    const header = request.headers.get("x-prent-mode") ?? "consultorio";
    const mode = (MODES as readonly string[]).includes(header) ? header : "consultorio";
    return {
      authenticator: "app",
...
export default eveChannel({
  ...
  // and every session would default to "consultorio". prentModeAuth accepts all callers (header-less
  // ones, like the eve TUI, default to "consultorio"), so the entries below are documentation of the
